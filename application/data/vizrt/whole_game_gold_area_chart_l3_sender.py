...
class WholeGameGoldAreaChartL3VizSender(DataEventDispatcher):
    ...
    abridged_data = kp.ListProperty()

    chart_min = kp.NumericProperty(0)
    chart_max = kp.NumericProperty(0)

    abridged_chart_min = kp.NumericProperty(0)
    abridged_chart_max = kp.NumericProperty(0)
    ...
    def on_game_reset(self, *args):
        ...
        self.abridged_data.clear()
        self.abridged_chart_min = 0
        self.abridged_chart_max = 0
    ...
    def format_viz_graph(self, *args):
        ...
        self.num_cols = (time_delta // 300000) + 1
        ...
        #Cut down data so viz can handle it
        this_data = self.chart_data

        while len(this_data) > 600:
            this_data = this_data[::2]

        if len(this_data) > 0:

            self.abridged_chart_min = min(this_data)
            self.abridged_chart_max = max(this_data)

        self.abridged_data = this_data

        if self.abridged_chart_max >= 0 and self.abridged_chart_min >= 0:
            self.zero_line_pos = 0

        elif self.abridged_chart_max < 0 and self.abridged_chart_min < 0:
            self.zero_line_pos = 100

        else:
            total = abs(self.abridged_chart_min) + abs(self.abridged_chart_max)
            self.zero_line_pos = abs(self.abridged_chart_min) / total * 100
    ...
    def update_vizrt(self, *args):
        ...
        string_data = ",".join([str(x) for x in self.abridged_data])
